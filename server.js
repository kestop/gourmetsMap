...
function getQuery(url) {
    var n = url.indexOf('?');
    var arg = url.substring(0,n);
    console.log("arg is " + arg);


    if(arg == "/register") {
        if(register(url.substring(n+1))){
            console.log("true");
            return true;
        }else {
            console.log("false");
    ...
    console.log("getQuery to the end");
    return false;
}
...
function register(url) {
    ...
    for (var i = 0; i < parts.length; i++) {
        paras[i] = parts[i].substring(parts[i].indexOf('=')+1);
    }


    paras[1] = paras[1].replace("%40","@");
    console.log(paras);
    ...
    var ps = db.prepare("INSERT INTO Person (email, username, password) VALUES (?,?,?)");
    
    ps.run(paras[1],paras[0],paras[2]);
    ps.finalize();
